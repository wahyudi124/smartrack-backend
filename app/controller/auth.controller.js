...
exports.signin = (req, res) => {
  ...
  User.findOne({
    ...
    res.status(200).send({"user":{ id: user.id, auth: true, token: token, role: user.role, username: user.username, monitor: user.monitor, control: user.control, config: user.config }});
